...
class CustomedFusionLayer(nn.Module):
    def __init__(self, type=None, in_size=0, out_size=0, head_nums=1, attention_dropout=0.1):
        ...
        if type == 'interpolate_0' or type == 'interpolate_others':
            self.noisy_weight = nn.Parameter(torch.tensor(0.5))
    ...
    def forward(self, enh:torch.Tensor, noisy:torch.Tensor):
        if self.type is None:
            ...
            # first layer: B x C X T -> B x T x C -> B x C x T
            enh_ = enh
            enh = enh + noisy * self.noisy_weight
            enh = self.layer_norm(enh.transpose(1,2)).transpose(1,2) + enh_
            return enh
        if self.type == 'interpolate_others':
            ...
            # first layer: B x C X T -> B x T x C -> B x C x T
            enh_ = enh
            enh = self.fusion_layer(torch.cat([enh, noisy], dim=1).transpose(1,2))
            enh = self.layer_norm(enh).transpose(1,2) + enh_
            return enh
            ...
            # first layer: B x C X T -> T x B x C -> B x C x T
...
class HubertConfig(FairseqDataclass):
    ...
    fusion_layer: Optional[str] = field(
        default=None, # 'mean', 'concat', 'add', 'attention', 'interpolate'
...
class HubertModel(BaseFairseqModel):
    ...
    def forward(
        ...
        features_ = features.transpose(1, 2)
        features_pen = features.float().pow(2).mean()
        # FIXME
        ...
        # residual
        features = self.layer_norm(features)
        # features = self.layer_norm(features) + features_
